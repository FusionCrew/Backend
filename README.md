...
### 📌 이다하 (Admin Auth, Inventory, User Cart/Order)

**담당 기능:**

- **일반**: 장바구니 CRUD, 주문 생성/조회/취소/확정, 대기표 발급 요청/조회
- **관리**: 관리자 로그인/로그아웃/토큰/내정보, 관리자 계정 CRUD, 재고 CRUD, 품절 처리
**작업 경로:** `domain/auth/`, `domain/stock/`, `domain/cart/`, `domain/order/`, `domain/ticket/`
